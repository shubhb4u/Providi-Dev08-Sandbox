...
/*
* edited by Marina Khakhayeva
*   added options:
*     pagination: 'withArrows'
*       (for arrows and pagination to display inline)
*   modified options:
*     trimSpace: 'move'
*       supports pagination
* */
  ...
  var Track = {
    ...
    go: function go(destIndex, newIndex, silently) {
      ...
      if (Math.abs(newPosition - this.position) >= 1 || isFade) {
        /* MK: modified to support pagination with trimSpace: 'move' */
        if (destIndex !== prevIndex && Splide.options.trimSpace === 'move') {
          var last = Splide.length - 1;
          var trimIndex = getTrimmedPosition(destIndex) === getTrimmedPosition(0) ? 0 :
            getTrimmedPosition(destIndex) === getTrimmedPosition(last) ? last :
              destIndex;
          Components.Controller.go(trimIndex, silently);
        } else {
    ...
    /* MK: added to support pagination with trimSpace: 'move' */
    gtp: function gtp(i) {
      return getTrimmedPosition(i);
    },
...
/* harmony default export */ var click = (function (Splide, Components) {
  ...
  function onClick(e) {
    if (disabled) {
      /* MK: check if clicks are frozen and restart event flow */
...
/* harmony default export */ var components_pagination = (function (Splide, Components, name) {
  ...
  var Pagination = {
    ...
    mount: function mount() {
      ...
      if (pagination) {
        /* MK: modified for withArrows */
        data = createPagination();
        var slider = Elements.slider;
        var withArrows = pagination === 'withArrows';
        var nextArrow = Splide.root.querySelector('.splide__arrow--next');
        var parent = pagination === 'slider' && slider ? slider : Splide.root;
	      if (withArrows) {
	        before(data.list, nextArrow);
        } else {
        ...
        /* MK: modified to support pagination with trimSpace: 'move' */
        if (Splide.options.trimSpace === 'move') {
          hideSkipped();
          addEventListener('resize', hideSkipped);
        }
      }
  ...
  function hideSkipped() {
    var pdl = Splide.Components.Pagination.data.list.childNodes;
    pdl.forEach(function(c, i, l){
      var last = l.length - 1;
      if (i === 0 || i === last) return false;
      var cp = Splide.Components.Track.gtp(i);
      var zp = Splide.Components.Track.gtp(0);
      var lp = Splide.Components.Track.gtp(last);
      if (cp === zp || cp === lp) {
        c.className = 'hidden';
      } else {
        c.className = '';
